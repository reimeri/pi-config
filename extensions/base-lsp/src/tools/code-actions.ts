  ...
  pi.registerTool({ name: "lsp_code_actions", label: "LSP Code Actions", description: "List and preview server code actions; apply only a freshly revalidated action previously previewed by actionId. Commands are never executed.", promptSnippet: "Preview or safely apply LSP quick fixes and refactors", promptGuidelines: ["Use lsp_code_actions with apply omitted first; apply only the returned actionId after reviewing the preview."], parameters: codeActionsSchema,
    async execute(_id, params: CodeActionsInput, signal) {
      ...
      try {
        ...
        const candidates: Array<{ action: CodeAction; manifest?: EditManifest; actionId: string; supported: boolean; reason?: string }> = [];
        for (const raw of actions) {
          ...
          const reason = action.disabled?.reason ?? (formatting ? "Formatting actions are excluded" : action.command && action.edit ? "Actions containing both edit and command cannot be applied" : action.command && !action.edit ? "Command-only actions are unsupported" : undefined);
          const manifest = action.edit ? await normalizeWorkspaceEdit(action.edit, runtime.boundary, capabilities.positionEncoding, runtime.loaded.config.limits, documentVersions) : undefined;
          const actionId = stableHash({ server: source.route.server.id, root: source.route.root, hash: source.snapshot.contentHash, title: action.title, kind: action.kind, edit: manifest?.fingerprint, command: action.command });
          candidates.push({ action, ...(manifest ? { manifest } : {}), actionId, supported: !reason && Boolean(manifest?.applicable), ...(reason ? { reason } : manifest && !manifest.applicable ? { reason: manifest.reasons.join("; ") } : {}) });
        }
        ...
        if (params.apply) {
          ...
          return envelope(`Applied ${selected.action.title} to ${changed.length} file(s).`, { ...baseDetails("code_actions", "ok", started), server: source.route.server.id, root: source.route.root, actionId: selected.actionId, applied: true, changed });
        }
        ...
        if (selected?.manifest) runtime.previewActions.add(selected.actionId);
        ...
        const status = params.title && candidates.length > 1 ? "ambiguous" : candidates.length ? "ok" : "no_results";
        const omittedActions = Math.max(0, totalActions - candidates.length);
        return envelope(text, { ...baseDetails("code_actions", status, started), server: source.route.server.id, root: source.route.root, actions: candidates.map((candidate) => ({ actionId: candidate.actionId, title: candidate.action.title, kind: candidate.action.kind, preferred: candidate.action.isPreferred ?? false, disabled: candidate.action.disabled, supported: candidate.supported, reason: candidate.reason, hasEdit: Boolean(candidate.action.edit), hasCommand: Boolean(candidate.action.command), previewed: candidate === selected })), totalActions, omittedActions, ...(selected ? { actionId: selected.actionId, preview: preview?.text, applicable: selected.supported } : {}), truncated: (preview?.truncated ?? false) || omittedActions > 0, warnings: omittedActions ? [`${omittedActions} action(s) omitted by the hard limit.`] : [] });
