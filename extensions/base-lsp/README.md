...
`waitMs` is the aggregate collection budget for the whole tool call after discovery/routing, including lazy client acquisition and all selected server/root groups. It is not a per-file allowance. When it expires, completed evidence is retained and unfinished files are reported as `timed_out` or `unconfirmed` rather than extending the call with another file/chunk timeout. An inventory larger than one batch is divided into chunks that each receive an equal share of the remaining budget, and a chunk that settles early returns its unused share to the next one, so a slow chunk cannot leave later files unchecked. `waitMs` bounds how long the call waits for a server, never how long a server is allowed to start: a server still initializing when the budget runs out keeps starting in the background and is ready for the next call.

Confirmed results are reused only while the server's view of the workspace is unchanged. Because a file's diagnostics can depend on its imports, synchronizing any document with content the server has not seen before discards the cached results for that server, and `refresh: true` bypasses the cache outright. Reopening an unmodified document after an LRU close is not a change and keeps the cache intact.
